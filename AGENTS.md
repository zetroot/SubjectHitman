...
- Миграции EF: `dotnet ef migrations add <Name> --project src/SubjectHitman.Api` (есть `DesignTimeDbContextFactory`). Именование БД — snake_case через `EFCore.NamingConventions`.
...
- `src/SubjectHitman.Api` — единственный хост: HTTP + консюмеры + сага в одном процессе.
