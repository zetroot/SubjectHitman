...
dotnet test SubjectHitman.slnx                       # 78 unit + 19 integration
...
- **NuGet Central Package Management (CPM):** версии всех пакетов централизованы в `Directory.Packages.props` (22 пакета). В `PackageReference` в `.csproj` атрибут `Version` не указывается.
...
- Prometheus: `/metrics` через `OpenTelemetry.Exporter.Prometheus.AspNetCore` (prerelease `1.16.0-beta.1`). В `WithMetrics` подключён `AddMeter("Wolverine")` — встроенные метрики Wolverine (счётчики `wolverine-messages-succeeded`, `wolverine-execution-failure`, `wolverine-dead-letter-queue`, гистограммы `wolverine-execution-time`, `wolverine-effective-time`, gauges `wolverine-inbox-count`, `wolverine-outbox-count`, `wolverine-scheduled-count`).
...
- Health checks: `AddWolverine("wolverine")` + `AddWolverineListeners("wolverine-listeners")` через `WolverineFx.HealthChecks` — мониторинг состояния Wolverine runtime и RabbitMQ listener'ов на `/health`.
