...
- **NuGet Central Package Management (CPM):** версии всех пакетов централизованы в `Directory.Packages.props` (19 пакетов). В `PackageReference` в `.csproj` атрибут `Version` не указывается.
...
- Метрики определены в классах-публикаторах (`Telemetry/`): `IApiMetricsPublisher` (Api) и `IDataAccessMetricsPublisher` (DataAccess). Каждый публикатор — singleton, инжектится через интерфейс.
